...
// Source: https://github.com/DusanKasan/parsemail
...
func parseMultipartAlternative(msg io.Reader, boundary string) (textBody, htmlBody string, embeddedFiles []EmbeddedFile, err error) {
	...
	for {
		...
		contentType, params, err := mime.ParseMediaType(part.Header.Get("Content-Type"))
		if err != nil {
			return textBody, htmlBody, embeddedFiles, fmt.Errorf("parsing media type: %s", err)
		}
		...
		case contentTypeTextPlain:
			ppContent, err := ioutil.ReadAll(part)
			if err != nil {
				return textBody, htmlBody, embeddedFiles, fmt.Errorf("parsing plain text part: %s", err)
			}
		...
		case contentTypeTextHtml:
			ppContent, err := ioutil.ReadAll(part)
			if err != nil {
				return textBody, htmlBody, embeddedFiles, fmt.Errorf("parsing html part: %s", err)
			}
		...
		case contentTypeMultipartRelated:
			...
			if err != nil {
				return textBody, htmlBody, embeddedFiles, fmt.Errorf("parsing related part: %s", err)
			}
		...
		default:
			if isEmbeddedFile(part) {
				ef, err := decodeEmbeddedFile(part)
				if err != nil {
					return textBody, htmlBody, embeddedFiles, fmt.Errorf("parsing embedded file: %s", err)
				}
